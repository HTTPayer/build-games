"""
analytics_dashboard.py — Streamlit analytics dashboard for the Composed protocol.
...
st.set_page_config(
    page_title="Composed Protocol Analytics",
    page_icon="📡",
    layout="wide",
)
...
st.sidebar.title("Composed Protocol Analytics")
...
if page == "Overview":
    ...
    c6.metric("Slash Rate", f"{ch.get('slash_rate', 0):.1%}")
...
# ---------------------------------------------------------------------------
    ...
    if selected_id is not None:
        ...
        if detail:
            ...
            col1.write(f"**Revenue Share:** `{detail.get('revenue_share', '')}`")
...
# ---------------------------------------------------------------------------
    ...
    if by_provider:
        ...
        split_data = {
            "Category": ["Protocol", "Rev Share", "Provider Direct"],
            "Amount": [protocol_total, rev_share_total, provider_direct_total],
        }
        fig = px.pie(
            split_data,
            names="Category",
