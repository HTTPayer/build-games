"""
analytics_api.py — FastAPI analytics REST API for the Composed protocol.
...
app = FastAPI(title="Composed Analytics API", version="1.0.0")
...
def list_providers() -> list[dict[str, Any]]:
    with db() as conn:
        rows = conn.execute(
            "SELECT id, owner, splitter, revenue_share, ts FROM providers ORDER BY id"
        ).fetchall()
...
@app.get("/rs/{rs_address}")
def rs_detail(rs_address: str) -> dict[str, Any]:
    with db() as conn:
        provider_row = conn.execute(
            "SELECT id, splitter FROM providers WHERE LOWER(revenue_share) = LOWER(?)", (rs_address,)
        ).fetchone()
        ...
        claims = conn.execute(
            "SELECT COALESCE(SUM(amount),0), COUNT(*) FROM revenue_claims WHERE LOWER(revenue_share) = LOWER(?)",
            (rs_address,),
        ).fetchone()

    return {
        "revenue_share": rs_address,
        "provider_id": provider_id,
        "splitter": splitter_address,
        "total_revenue_raw": revenue_raw,
        "total_revenue_usdc": usdc(revenue_raw),
        "total_claimed_raw": claims[0],
        "total_claimed_usdc": usdc(claims[0]),
        "claim_count": claims[1],
    }
