...
class ComposedClient:
    ...
    def deploy_provider(
        ...
        """Deploy a new provider with RS token and splitter."""
        self._require_signer()
        ...
        receipt = self._send_tx(
            self._factory.functions.deployProvider(
                ...
                symbol,
                rs_shares,
                Web3.to_checksum_address(rs_recipient) if rs_recipient else ZERO,
                Web3.to_checksum_address(provider_treasury) if provider_treasury else ZERO,
                provider_bp,
                metadata_uri,
            ),
        ...
        deployed_logs = self._factory.events.ProviderDeployed().process_receipt(receipt, errors=DISCARD)
        ...
        revenue_share = args["revenueShare"]
        splitter = args["splitter"]
        actual_rev_share_bp = args["revenueShareBp"]
        actual_provider_bp = args["providerTreasuryBp"]

        # Parse ProviderRegistered from registry.
        reg_logs = self._registry.events.ProviderRegistered().process_receipt(receipt, errors=DISCARD)
        ...
        return DeployedProvider(
            ...
            provider_bp=actual_provider_bp,
    ...
    def register_endpoint(
        ...
        """Register an endpoint with payment metadata.

        If metadata fields are not provided, fetches from the endpoint.
        The contract validates that payTo matches the provider's revenueSplitter.
        """
        self._require_signer()

        from .hash import fetch_payment_metadata

        if not all([pay_to, asset, network, amount]):
            payment_data = fetch_payment_metadata(url)
        ...
        receipt = self._send_tx(
            self._registry.functions.registerEndpoint(
                ...
                path,
                method.upper(),
                self.w3.to_checksum_address(pay_to),
        ...
        reg_logs = self._registry.events.EndpointRegistered().process_receipt(receipt, errors=DISCARD)
        if not reg_logs:
            raise RuntimeError(
    ...
    def update_endpoint(
        ...
        If metadata fields are not provided, fetches from the endpoint URL.
        The contract validates that payTo matches the provider's revenueSplitter.
        ...
        if not ep.path:
            raise ValueError(
                "Metadata required — endpoint path is empty on-chain."
            )

        if not all([pay_to, asset, network, amount]):
            from .hash import fetch_payment_metadata
            payment_data = fetch_payment_metadata(ep.path)
            pay_to   = pay_to   or payment_data["payTo"]
            asset    = asset    or payment_data["asset"]
            network  = network  or payment_data["network"]
            amount   = amount   or str(payment_data["amount"])
        ...
        receipt = self._send_tx(
            self._registry.functions.updateEndpoint(
                eid_bytes,
                self.w3.to_checksum_address(pay_to),
                self.w3.to_checksum_address(asset),
                network,
