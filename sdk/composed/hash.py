...
def fetch_integrity_hash(endpoint_url: str, timeout: int = 10) -> str:
    ...
    payment_data = fetch_payment_metadata(endpoint_url, timeout=timeout)
    return compute_integrity_hash(payment_data)


def fetch_payment_metadata(endpoint_url: str, timeout: int = 10) -> dict:
    """Fetch x402 payment metadata from an endpoint. Returns raw fields dict."""
    ...
    return _parse_payment_data(status, headers, body_bytes, endpoint_url)
