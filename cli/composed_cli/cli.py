...
Commands:
  ...
  deploy-provider    Deploy RS token + splitter, register in APIIntegrityRegistry
  ...
  splitter           Show splitter state, optionally distribute
...
Usage:
  ...
  uv run python cli.py deploy-provider --name "Weather API" --symbol "wAPI" --rs-shares 1000000000000
...
from .x402_metadata import fetch_integrity_hash, fetch_payment_metadata
...
# ABIs for contracts not in the SDK (splitter, revenue-share)
_splitter_abi = None
_rs_abi       = None
...
def _my_deployments() -> list[dict]:
    ...
    while start <= latest:
        ...
        try:
            ...
            for log in logs:
                d = dict(log["args"])
                # Try to find matching ProviderRegistered event for registry ID
                tx_hash = log["transactionHash"]
                try:
                    receipt = _client.w3.eth.get_transaction_receipt(tx_hash)
                    reg_logs = _client.registry.events.ProviderRegistered().process_receipt(receipt, errors=DISCARD)
                    if reg_logs:
                        d["registry_id"] = reg_logs[0]["args"]["id"]
                    else:
                        d["registry_id"] = None
                except Exception:
                    d["registry_id"] = None

                # Fetch endpoints registered by this provider
                d["endpoints"] = []
                registry_id = d.get("registry_id")
                if registry_id:
                    try:
                        # Get provider's owner address to query providerEndpoints
                        provider = _client.registry.functions.providers(registry_id).call()
                        provider_owner = provider[0]  # owner field
                        # Try to get endpoints - mapping is providerEndpoints(address, uint256) -> bytes32
                        ep_count = 10  # reasonable max to try
                        for idx in range(ep_count):
                            try:
                                eid = _client.registry.functions.providerEndpoints(provider_owner, idx).call()
                                if eid == "0x" + "00" * 32:
                                    break
                                ep = _client.registry.functions.endpoints(eid).call()
                                if ep[6]:  # active
                                    # Convert bytes32 to hex string
                                    eid_hex = eid if isinstance(eid, str) else "0x" + eid.hex()
                                    d["endpoints"].append({
                                        "endpointId": eid_hex,
                                        "path": ep[2],
                                        "method": ep[3],
...
def _resolve_revenue_share(arg: str | None) -> str:
    if arg:
        return _client.w3.to_checksum_address(arg)
    deployments = _my_deployments()
    rs_addrs = [d["revenueShare"] for d in deployments]
    if not rs_addrs:
...
def _print_provider_stats(splitter_addr: str, rs_addr: str) -> None:
...
def cmd_deploy_provider(args):
    provider_bp = args.provider_bp
    rs_share_bp = _MAX_PROVIDER_BP - provider_bp

    if provider_bp < 0 or provider_bp > _MAX_PROVIDER_BP:
        sys.exit(f"provider-bp must be 0 to {_MAX_PROVIDER_BP}")

    if args.rs_shares < 1_000_000:
        sys.exit(f"rs-shares must be at least 1_000_000 (1 full share)")

    print(f"\n── deploy-provider ───────────────────────────────────────────────")
    print(f"  name          : {args.name} ({args.symbol})")
    print(f"  rs shares     : {args.rs_shares / 1e6:.0f} (min 1)")
    print(f"  provider bp   : {provider_bp / 100:.2f}%")
    print(f"  RS holders    : {rs_share_bp / 100:.2f}%")
    ...
    deployed = _client.deploy_provider(
        ...
        rs_recipient=args.rs_recipient or None,
        provider_treasury=args.provider_treasury or None,
        provider_bp=provider_bp,
    ...
    verify_contract("ProviderRevenueSplitter", splitter_addr, abi_encode(
        ...
        [usdc_address, protocol_treasury, _PROTOCOL_BP, _client.account.address,
         args.provider_treasury or ZERO, provider_bp, rs_addr, rs_share_bp],
    ).hex())
...
def cmd_register_endpoint(args):
    ...
    # Fetch payment metadata from endpoint if not all fields provided
    if not all([args.pay_to, args.asset, args.network, args.amount]):
        from .x402_metadata import fetch_payment_metadata

        print(f"\n  fetching payment metadata from live server…")
        metadata = fetch_payment_metadata(args.url, verbose=True)
        args.pay_to = args.pay_to or metadata.get("payTo", "")
        args.asset = args.asset or metadata.get("asset", "")
        args.network = args.network or metadata.get("network", "")
        args.amount = args.amount or metadata.get("amount", "")
        args.url = args.url or metadata.get("url", "")

    # Convert to checksum addresses (required by web3)
    # Use address(0) if not provided (will default to revenueSplitter)
    pay_to_addr = _client.w3.to_checksum_address(args.pay_to) if args.pay_to else "0x" + "00" * 20
    asset_addr = _client.w3.to_checksum_address(args.asset) if args.asset else "0x" + "00" * 20

    print(f"\n→ registerEndpoint(providerId={args.provider_id}, {args.method} {args.url})")
    print(f"  payTo={pay_to_addr} asset={asset_addr} network={args.network} amount={args.amount}")
    receipt = send_tx(
        _client.w3, _client.account,
        _client.registry.functions.registerEndpoint(
            args.provider_id, args.url, args.method,
            pay_to_addr, asset_addr, args.network, args.url, int(args.amount or 0)
        ),
        "registerEndpoint",
    )
...
def cmd_update_endpoint(args):
    ...
    # Fetch new metadata from endpoint if not all fields provided
    if not all([args.pay_to, args.asset, args.network, args.amount]):
        print(f"\n  fetching payment metadata from live server…")
        metadata = fetch_payment_metadata(ep.path, verbose=True)
        args.pay_to = args.pay_to or metadata.get("payTo", "")
        args.asset = args.asset or metadata.get("asset", "")
        args.network = args.network or metadata.get("network", "")
        args.amount = args.amount or metadata.get("amount", "")
        ep.path = ep.path or metadata.get("url", "")
    ...
    tx_hash = _client.update_endpoint(
        args.endpoint_id,
        pay_to=pay_to_addr,
        asset=asset_addr,
    ...
    print(f"  tx: {_tx_url(tx_hash)}")
    print(f"  ✓ endpoint updated  (v{ep.version + 1})")
    print("──────────────────────────────────────────────────────────────────\n")
...
def cmd_splitter(args):
    ...
    print(f"  providerTreasury  : {prov or '(none)'}  ({prov_bp / 100:.2f}%)")
...
def cmd_revenue_share(args):
    ...
    if args.claim:
        if claimable == 0:
            ...
            send_tx(_client.w3, _client.account, rs.functions.claim(_client.account.address), "claim")
            print("  done\n")
    else:
        print()
...
def cmd_status(args):
    ...
    deployments = _my_deployments()
    if not deployments:
        print(f"\n  (no ProviderDeployed events found for {account.address[:10]}... — register endpoints manually)")
    if deployments:
        print(f"\n── Your Deployed Providers ({len(deployments)}) ──────────────────────────────────")
        for i, d in enumerate(deployments, 1):
            registry_id = d.get("registry_id")
            print(f"\n  ── Provider {i} {f'(registry #{registry_id})' if registry_id else ''} ──")
            _print_provider_stats(d["splitter"], d["revenueShare"])
            endpoints = d.get("endpoints", [])
            if endpoints:
                print(f"  endpoints:")
                for ep in endpoints:
                    print(f"    {ep['method']:6} {ep['endpointId']} → {ep['path']}")
            else:
                print(f"  endpoints: (none)")
...
Composed Protocol CLI
  ...
  Inspection
    ...
    revenue-share         RS supply, EPS, your claimable USDC; optionally claim
...
def main():
    ...
    # deploy-provider
    dp = sub.add_parser("deploy-provider", help="Deploy RS token + splitter and register provider")
    dp.add_argument("--name",             required=True,  help="RS token name")
    dp.add_argument("--symbol",           required=True,  help="RS token symbol")
    dp.add_argument("--rs-shares",        type=int, required=True, help="Genesis shares for RS (min: 1_000_000 = 1 full share)")
    dp.add_argument("--provider-bp",      type=int, default=0,     help="Basis points to provider treasury (default 0)")
    dp.add_argument("--rs-recipient",     default="",  help="RS genesis recipient (default: signer)")
    dp.add_argument("--provider-treasury",default="",  help="Address for provider treasury (required if --provider-bp > 0)")
    ...
    # update-endpoint
    ue = sub.add_parser("update-endpoint", help="Update endpoint payment metadata")
    ue.add_argument("--endpoint-id", required=True, help="Endpoint ID (bytes32 hex)")
    ue.add_argument("--pay-to",      default="", help="Payment address (omit to use provider's revenueSplitter)")
    ue.add_argument("--asset",       default="", help="Payment asset address (e.g. USDC)")
    ue.add_argument("--network",      default="", help="CAIP network ID (e.g. eip155:43113)")
    ue.add_argument("--amount",       default="", help="Payment amount in smallest units")
    ...
    re.add_argument("--pay-to",      default="", help="Payment address (omit to use provider's revenueSplitter)")
    re.add_argument("--asset",       default="", help="Payment asset address (e.g. USDC)")
    re.add_argument("--network",      default="", help="CAIP network ID (e.g. eip155:43113)")
    re.add_argument("--amount",       default="", help="Payment amount in smallest units")
    ...
    # splitter
    sp = sub.add_parser("splitter", help="Show splitter state, optionally distribute")
