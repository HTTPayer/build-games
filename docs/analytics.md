...
- `contracts/scripts/.env` with `GATEWAY_URL` set (Alchemy RPC for Fuji)
- Extra Python deps installed (once):

```bash
cd contracts/scripts
uv add fastapi "uvicorn[standard]" streamlit plotly pandas
```

### Step 1 — Initial sync

Run from `contracts/scripts/`:
...
```bash
# still in contracts/scripts/
uv run uvicorn analytics_api:app --port 8000 --reload
```
