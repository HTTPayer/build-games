...
ChallengeManager, and per-provider contracts (vault, splitter, revenue share)
...
# Minimal ERC4626 ABI for Deposit and Withdraw events (standard OZ)
ERC4626_EVENTS_ABI: list[dict] = [
    {
        "name": "Deposit",
        "type": "event",
        "inputs": [
    ...
    for ev in events:
        ...
        try:
            conn.execute(
                ...
                (id, owner, vault, splitter, revenue_share, deployer,
                 vault_bp, rev_share_bp, provider_bp, protocol_bp,
                 block_number, tx_hash, ts)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    None,  # id filled in by ProviderRegistered join
                    args.get("vaultGenesisRecipient"),
                    Web3.to_checksum_address(args["vault"]),
...
def process_registry(
    ...
    for ev in ep_events:
        ...
        row = conn.execute(
            "SELECT id FROM providers WHERE owner = ? OR vault = ?",
            (ep_provider_addr, ep_provider_addr),
        ).fetchone()
...
def process_per_provider(
    ...
    providers = conn.execute(
        "SELECT id, vault, splitter, revenue_share FROM providers WHERE vault IS NOT NULL"
    ).fetchall()
    ...
    for row in providers:
        provider_id, vault_addr, splitter_addr, rev_share_addr = row
            ...
            for ev in dist_events:
                ...
                conn.execute(
                    ...
                    (splitter, provider_id, total, protocol_share, provider_share,
                     rev_share_share, vault_share, block_number, tx_hash, ts)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        splitter_addr,
        ...
        # --- Vault: Deposit and Withdraw (ERC4626 standard events) ---
        try:
            vault = w3.eth.contract(
                address=Web3.to_checksum_address(vault_addr), abi=ERC4626_EVENTS_ABI
            )
            deposit_events = fetch_events(w3, vault, "Deposit", from_block, to_block)
            for ev in deposit_events:
                ...
                conn.execute(
                    ...
                    (vault, provider_id, sender, owner, assets, shares,
                     block_number, tx_hash, ts)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
            ...
            withdraw_events = fetch_events(w3, vault, "Withdraw", from_block, to_block)
            for ev in withdraw_events:
                args = ev["args"]
                block_num = ev["blockNumber"]
                tx_hash = ev["transactionHash"].hex()
                ts = get_block_ts(w3, block_num)
                conn.execute(
                    ...
                    (vault, provider_id, sender, receiver, owner, assets, shares,
                     block_number, tx_hash, ts)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        ...
                        provider_id,
                        Web3.to_checksum_address(args["sender"]),
                        Web3.to_checksum_address(args["receiver"]),
                        Web3.to_checksum_address(args["owner"]),
                        int(args["assets"]),
                        int(args["shares"]),
                        block_num,
            ...
            print(f"    [warn] vault {vault_addr} events: {exc}")
